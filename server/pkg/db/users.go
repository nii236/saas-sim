...
var (
	userColumns               = []string{"id", "name", "archived", "archived_at", "updated_at", "created_at"}
	userColumnsWithoutDefault = []string{"name", "archived_at"}
...
func (userL) LoadTodos(e boil.Executor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`todos`), qm.WhereIn(`user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *User) AddTodos(exec boil.Executor, insert bool, related ...*Todo) error {
	var err error
