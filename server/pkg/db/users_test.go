...
func testUserToManyTodos(t *testing.T) {
	...
	slice := UserSlice{&a}
	if err = a.L.LoadTodos(tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	a.R.Todos = nil
	if err = a.L.LoadTodos(tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Todos); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
var (
	userDBTypes = map[string]string{`ID`: `uuid`, `Name`: `text`, `Archived`: `boolean`, `ArchivedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
